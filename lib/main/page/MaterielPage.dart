...
class _TextFieldDemoState extends State<TextFieldDemo> {
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        WidgetBase.rowTextTwo(
                            "当前重量", "$materielQuantrestante$materielUnittare"),
                      ],
                    ),
                  ),
                  offstage: boolOff,
  ...
  Future<dynamic> getEQP(String eqpString) async {
    var url = httpURL + "custom/Pda/read_idy";
    var response =
        await http.post(url, body: json.encode({'BARCODE': eqpString}));
    ...
    if (response.statusCode == 200) {
      if (code == 0) {
        ...
        setState(() {
          ...
          runCode = 1;
          materielCodeart = json.decode(response.body)['data']['CODEART'];
          materielErpLot = json.decode(response.body)['data']['ERP_LOT_NUMBER'];
          materielQuantrestante =
              json.decode(response.body)['data']['QUANTRESTANTE'];
          materielUnittare = json.decode(response.body)['data']['UNITESTOCK'];
        });
        return response;
      } else {
        setState(() {
